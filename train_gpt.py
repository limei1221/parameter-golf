...
class Hyperparameters:
    ...
    late_qat_threshold = float(os.environ.get("LATE_QAT_THRESHOLD", 0.0))  # 0 = QAT from start; e.g. 0.15 = enable when LR scale < 0.15
    tie_embeddings = bool(int(os.environ.get("TIE_EMBEDDINGS", "1")))
    rope_base = float(os.environ.get("ROPE_BASE", 10000.0))
    rope_dims = int(os.environ.get("ROPE_DIMS", 0))  # 0 = full RoPE; e.g. 16 = partial (16 of head_dim)
    ln_scale = bool(int(os.environ.get("LN_SCALE", "0")))  # 1/sqrt(layer+1) damping on RMSNorm outputs
    ...
    wandb_project = os.environ.get("WANDB_PROJECT", "parameter-glof-a100")
...
def quantize_intN_per_row(t: Tensor, clip_range: int = 31, gptq_lite: bool = False) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        if gptq_lite:
            # GPTQ-lite: search clip percentiles per layer, pick lowest reconstruction MSE
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], gptq_lite: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            # Fallback: int8 for non-targeted categories
...
def _ste_fake_quantize(w: Tensor, clip_range: int) -> Tensor:
    """STE fake quantization: forward uses quantized values, backward passes through."""
    amax = w.abs().amax(dim=-1, keepdim=True).clamp_min(1e-12)
    scale = amax / clip_range
    q = (w / scale).round().clamp(-(clip_range + 1), clip_range)
    w_q = q * scale
    return w + (w_q - w).detach()  # STE: value of w_q, gradient of w


class CastedLinear(nn.Linear):
    # Keep weights in fp32 for optimizer/state quality, cast at matmul time for bf16 compute.
    # When _qat_clip > 0 and training, applies STE fake quantization.
    _qat_clip: int = 0  # 0 = no QAT; 15 = int5; 31 = int6

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self._qat_clip > 0 and self.training and w.ndim == 2:
            w = _ste_fake_quantize(w, self._qat_clip)
...
def apply_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    rd = cos.size(-1) * 2  # rope_dims inferred from cos cache
    if rd < x.size(-1):
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        ...
        B, T, H, D = y.shape
        Hkv = v.size(-2)  # number of key/value heads
        group = H // Hkv
        y_g = y.reshape(B, T, Hkv, group, D)
        vn = F.normalize(v, dim=-1).unsqueeze(-2)  # [B, T, Hkv, 1, D] — broadcast ready
        proj = (y_g * vn).sum(dim=-1, keepdim=True) * vn  # [B, T, Hkv, group, D]
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        # cos/sin are (1,1,seqlen,half) — reshape to (1,seqlen,1,half) for (B,S,H,D) layout
...
# xIELU (Expanded Integral of the Exponential Linear Unit): Huang & Schlag,
# "Deriving Activation Functions Using Integration", arXiv:2411.13010 — https://arxiv.org/abs/2411.13010
class XIELU(nn.Module):
    def __init__(self, alpha_p: float = 0.8, alpha_n: float = 0.8, beta: float = 0.5, eps: float = -1e-6):
        super().__init__()
        self.alpha_p = nn.Parameter(torch.log(torch.expm1(torch.tensor(alpha_p))).unsqueeze(0))
        self.alpha_n = nn.Parameter(torch.log(torch.expm1(torch.tensor(alpha_n - beta))).unsqueeze(0))
        self.register_buffer("beta", torch.tensor(beta))
        self.register_buffer("eps", torch.tensor(eps))

    def forward(self, x: Tensor) -> Tensor:
        ap = F.softplus(self.alpha_p)
        an = self.beta + F.softplus(self.alpha_n)
        return torch.where(
            x > 0,
            ap * x * x + self.beta * x,
            (torch.expm1(torch.min(x, self.eps)) - x) * an + self.beta * x,
        )


class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, activation: str = "relu2"):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(
        ...
        # Value Embedding: shared table with per-layer scales
        self.ve_layer_indices = [
            idx for idx in (int(x) for x in ve_layers.split(",") if x.strip())
            if idx < num_layers
        ] if ve_enabled else []
...
def main() -> None:
    ...
    if args.qat_enabled and args.late_qat_threshold <= 0:
        for name, module in base_model.named_modules():
            if isinstance(module, CastedLinear):
                if ".mlp." in name:
                    module._qat_clip = 15  # int5 for MLP
                elif ".attn." in name:
                    module._qat_clip = 31  # int6 for attention
    ...
    # Wandb init (only on rank 0, only if WANDB_PROJECT is set and wandb is installed)
    use_wandb = master_process and bool(args.wandb_project) and _wandb is not None
    if use_wandb:
        _wandb.init(
            project=args.wandb_project,
            name=args.run_id,
            config={k: v for k, v in vars(type(args)).items()
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if max_wallclock_ms is not None:
            ...
            return min(remaining_ms / max(warmdown_ms, 1e-9), 1.0)
        warmdown_steps = max(int(args.iterations * args.warmdown_frac), 1)
        warmdown_start = args.iterations - warmdown_steps
        if step >= warmdown_start:
            return max((args.iterations - step) / warmdown_steps, 0.0)
        return 1.0
    ...
    while True:
        ...
        if should_validate:
            ...
            if use_wandb:
                _wandb.log({"val_loss": val_loss, "val_bpb": val_bpb}, step=step)
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        # Late QAT: enable fake quantization when LR scale drops below threshold
        if args.qat_enabled and args.late_qat_threshold > 0 and scale < args.late_qat_threshold:
            for name, module in base_model.named_modules():
                if isinstance(module, CastedLinear) and module._qat_clip == 0:
                    if ".mlp." in name:
                        module._qat_clip = 15
                    elif ".attn." in name:
                        module._qat_clip = 31
                    if module._qat_clip > 0:
                        log0(f"late_qat:enabled {name} clip={module._qat_clip} step={step} scale={scale:.4f}")
        ...
        if should_log_train:
            ...
            if use_wandb:
                _wandb.log({"train_loss": train_loss.item(), "lr_scale": scale, "muon_momentum": muon_momentum}, step=step)
    ...
    # -----------------------------
    # Mixed int5/int6 quantization + zstd compression to fit under 16MB.
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn", "bigram"}, gptq_lite=args.gptq_lite)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int5int6_{_COMPRESSOR}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_int5int6_{_COMPRESSOR}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")

    if use_wandb:
        _wandb.log({"final_val_loss": q_val_loss, "final_val_bpb": q_val_bpb})
        _wandb.finish()
